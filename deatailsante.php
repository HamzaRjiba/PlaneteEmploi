...
$totalOffres = $conn->query("SELECT COUNT(*) FROM sante")->fetch_row()[0];
$offresParPage=10;
$totalPages = ceil($totalOffres / $offresParPage);

$offset = ($currentPage - 1) * $offresParPage;
$sql = "SELECT * FROM sante ORDER BY created_at DESC LIMIT $offresParPage OFFSET $offset";
...
<div class="container mt-5">
    <h2 class="mb-4">Affichage des données de la table sante</h2>
    <div class="accordion" id="accordionExample">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $showClass = ($i === 0) ? 'show' : '';
                $noms = explode(";", $row["CV_filename"]);
                $nombreDElements = count($noms);
                ...
                if ($nombreDElements == 2) {

                echo '<a href="fichiers/' . $noms[0] . '" target="_blank">Ouvrir le CV</a> <br>';
                echo '<a href="fichiers/' . $noms[1] . '" target="_blank">Ouvrir le permis</a> <br>';

                }
                else{
                  echo '<a href="fichiers/' . $noms[0] . '" target="_blank">Ouvrir le CV</a> <br>';
                echo '<a href="fichiers/' . $noms[1] . '" target="_blank">Ouvrir le diplome</a> <br>';
                echo '<a href="fichiers/' . $noms[2] . '" target="_blank">Ouvrir le rcr</a> <br>';
                echo '<a href="fichiers/' . $noms[3] . '" target="_blank">Ouvrir le PDSB</a> <br>';
                }
